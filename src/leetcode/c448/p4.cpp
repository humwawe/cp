...
public:
  int magicalSum(int m, int K, vector<int> &nums) {
    ...
    // 预处理 nums[i] 的 j 次方
    // vector power(n, vector<Z>(m + 1, 0));
    // for (int i = 0; i < n; i++) {
    //   power[i][0] = 1;
    //   for (int j = 1; j <= m; j++) {
    //     power[i][j] = power[i][j - 1] * nums[i];
    //   }
    // }


    vector f(n, vector(m + 1, vector(m * 2 + 1, vector<Z>(K + 1, 0))));
    // 递推初值：枚举值 0 填到几个位置里
    for (int j = 0; j <= m; j++) {
      f[0][j][j][0] = comb.fac(m) * comb.invfac(j) * power<Z>(nums[0], j);
    }
    for (int i = 0; i + 1 < n; i++)
      for (int j = 0; j <= m; j++)
        for (int t = 0; t <= m * 2; t++)
          for (int k = 0; k <= K; k++)
            if (f[i][j][t][k].val()) {
              for (int c = 0; j + c <= m; c++) {
                ...
                int kk = t % 2 + k;
                if (kk <= K) {
                  f[i + 1][j + c][tt][kk] += f[i][j][t][k] * comb.invfac(c) * power<Z>(nums[i + 1], c);
                }
              }
            }

    Z ans = 0;
    // t 里 1 的数量，加上被剔除的位数里 1 的数量要恰为 K
